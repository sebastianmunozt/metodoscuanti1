# Prueba 2. VERSIÓN A - Métodos Cuantitativos en R
...
antropo_2023 <- read.xlsx(xlsxFile = "antropologia_2023.xlsx", startRow = 2)
...
antropo_2023 <- antropo_2023 %>% rename (edad = p02,
  genero = p03,
  annio = p04,
  comuna_actual = p05,
  comuna_pre= p06,
...
write.xlsx(x = antropo_2023,file = "antropo_2023_limpia.xlsx")
...
antropo_limpia <- read.xlsx(xlsxFile = "antropo_2023_limpia.xlsx")
...
base_nombres <- antropo_limpia %>% select(genero, edad, tipo_colegio, educacion_madre, clase_social_subjetiva)
...
unique(antropo_limpia$clase_social_subjetiva)
antropologia_clasemedia <- antropo_limpia %>% filter (clase_social_subjetiva == "Al grupo de clase social media")
...
table(antropologia_clasemedia$clase_social_subjetiva)
...
frecuencia_clase_social <- table(antropo_limpia$clase_social_subjetiva)
...
porcentajes_clases <- prop.table(frecuencia_clase_social) * 100
...
porcentaje_clases <- as.data.frame(porcentajes_clases)
...
ggplot(data =porcentaje_clases, aes(x =Var1, y =Freq, fill = Var1)) +
  geom_bar(stat = "identity", fill = "#faad3e") +
  theme(axis.text.x = element_text(angle = 70, hjust = 1)) +
  labs(title = "Gráfico de Barras de Clase social subjetiva", x = "Clase social", y = "%") +
  scale_y_continuous(labels = scales::percent_format(scale = 1))
